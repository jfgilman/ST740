...
simpExpoMCMC <- function(data, samples = 5000, shapePriorA = .001,
  ...
  logPost <- function(parm, d) {
    ...
    for(k in 1:length(d)){
      lp <- lp + 
        sum(dexp(d[[k]]$MBF[which(d[[k]]$Censor == 0 & d[[k]]$Phase == 1 & d[[k]]$trun == F)], parm[1], log = T)) +
        sum(dexp(d[[k]]$MBF[which(d[[k]]$Censor == 0 & d[[k]]$Phase == 2 & d[[k]]$trun == F)], parm[1]*parm[2], log = T)) +
        sum(dexp(d[[k]]$MBF[which(d[[k]]$Censor == 0 & d[[k]]$Phase == 3 & d[[k]]$trun == F)], parm[1]*parm[2]*parm[3], log = T)) -
        sum(parm[1]*(d[[k]]$MBF[which(d[[k]]$Censor == 1 & d[[k]]$Phase == 1 & d[[k]]$trun == F)])) -
        sum(parm[1]*parm[4]*(d[[k]]$MBF[which(d[[k]]$Censor == 1 & d[[k]]$Phase == 2 & d[[k]]$trun == F)])) -
        sum(parm[1]*parm[4]*parm[5]*(d[[k]]$MBF[which(d[[k]]$Censor == 1 & d[[k]]$Phase == 3 & d[[k]]$trun == F)])) +
        sum(parm[1]*(d[[k]]$Lower[which(d[[k]]$Phase == 1 & d[[k]]$trun == T)]) - 
              parm[1]*(d[[k]]$Upper[which(d[[k]]$Phase == 1 & d[[k]]$trun == T)])) +
        sum(parm[1]*parm[2]*(d[[k]]$Lower[which(d[[k]]$Phase == 2 & d[[k]]$trun == T)]) - 
              parm[1]*parm[2]*(d[[k]]$Upper[which(d[[k]]$Phase == 2 & d[[k]]$trun == T)])) + 
        sum(parm[1]*parm[2]*parm[3]*(d[[k]]$Lower[which(d[[k]]$Phase == 3 & d[[k]]$trun == T)]) - 
              parm[1]*parm[2]*parm[3]*(d[[k]]$Upper[which(d[[k]]$Phase == 3 & d[[k]]$trun == T)])) 
    }
