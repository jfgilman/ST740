...
simpWeiMCMC <- function(data, samples = 5000, shapePriorA = .001,
  ...
  for(i in 1:length(lam_finalDraws)){
    for(j in 1:length(data)){
      d[i] <- d[i] - 
        2*(sum(logWeibullSumCPP(data[[j]]$MBF[which(data[[j]]$Censor == 0 & data[[j]]$Phase == 1 & data[[j]]$trun == F)], beta_finalDraws[i], lam_finalDraws[i])) +
             sum(logWeibullSumCPP(data[[j]]$MBF[which(data[[j]]$Censor == 0 & data[[j]]$Phase == 2 & data[[j]]$trun == F)], beta_finalDraws[i], lam_finalDraws[i]*theta1_finalDraws[i])) +
             sum(logWeibullSumCPP(data[[j]]$MBF[which(data[[j]]$Censor == 0 & data[[j]]$Phase == 3 & data[[j]]$trun == F)], beta_finalDraws[i], lam_finalDraws[i]*theta1_finalDraws[i]*theta2_finalDraws[i])) -
  ...
  dthetahat <- 0
  for(j in 1:length(data)){
    dthetahat <- dthetahat - 2*(sum(logWeibullSumCPP(data[[j]]$MBF[which(data[[j]]$Censor == 0 & data[[j]]$Phase == 1 & data[[j]]$trun == F)], mean(beta_finalDraws), mean(lam_finalDraws))) +
                                  sum(logWeibullSumCPP(data[[j]]$MBF[which(data[[j]]$Censor == 0 & data[[j]]$Phase == 2 & data[[j]]$trun == F)], mean(beta_finalDraws), mean(lam_finalDraws)*mean(theta1_finalDraws))) +
                                  sum(logWeibullSumCPP(data[[j]]$MBF[which(data[[j]]$Censor == 0 & data[[j]]$Phase == 3 & data[[j]]$trun == F)], mean(beta_finalDraws), mean(lam_finalDraws)*mean(theta1_finalDraws)*mean(theta2_finalDraws))) -
